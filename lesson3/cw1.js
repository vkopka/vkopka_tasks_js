...
let simpsons = [
    ...
    },
    {
        ...
        info: 'Ма́рджори Жакли́н «Мардж» Си́мпсон (в девичестве Бувье́) (англ. Marjorie Jacqueline «Marge» Simpson) — ' +
            ...
            'на оранжевом универсале. У неё шикарные синие волосы, которые она обычно собирает в очень высокую ' +
    ...
    },
    {
        ...
        info: 'Ма́ргарет Эвелин «Мэ́гги» Си́мпсон (англ. Margaret Evelyn “Maggie” Simpson) — персонаж мультсериала ' +
            ...
            'Грейнинга. После появления в шоу Трейси Ульман, через три года семья Симпсонов получила собственный ' +
            'сериал на телеканале Fox, дебют произошёл 17 декабря 1989 года.',
        photo: 'https://upload.wikimedia.org/wikipedia/ru/9/9d/Maggie_Simpson.png'
    },
];
let templateSimpsonStr1 = '';
let templateSimpsonStr2 = '';
for (const simpson of simpsons) {
    templateSimpsonStr1 +=
        `<div class = "style-Simpson">
            ${simpson.name} - ${simpson.surname} - ${simpson.age} - ${simpson.info} <br>
            <img src="${simpson.photo}" alt="${simpson.name}">  <br>
        </div>`;
    templateSimpsonStr2 +=
        `<div class = "style-Simpson">
            <h3> ${simpson.name} ${simpson.surname}. age is - ${simpson.age} </h3> 
            <p> ${simpson.info} </p>
            <img src="${simpson.photo}" alt="${simpson.name}">  <br>
        </div>`;
}
document.write(templateSimpsonStr1 + templateSimpsonStr2);
...
for (const product of products) {
    templateProduct +=
        `<div class = "style-Product">
             ...
             <img class = "style-Image" src="${product.image}" alt="${product.title}"> <br>
        </div>`;
}
document.write(templateProduct);
